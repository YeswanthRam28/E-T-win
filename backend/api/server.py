...
@app.get("/")
def root():
    return {"status": "E<T>WIN Core Engine Online", "nodes": engine.pyg_data.num_nodes, "edges": engine.pyg_data.num_edges}
...
def run_simulation(req: PolicyRequest):
    ...
    return {"status": "success", "current_timestep": engine.current_timestep, "results": new_metrics}
...
def reset_simulation():
    global engine
    engine = SimulationEngine()
    return {"status": "reset_successful", "current_timestep": 0}
...
def get_nodes():
    ...
    for i in range(n):
        ...
        node_list.append({"id": i, "lat": float(lats[i]), "lon": float(lons[i]), "stress": composite, "emissions": ems, "vulnerability": vuln})
...
def policy_chat(req: PolicyChatRequest):
    ...
    if api_key:
        try:
            ...
            analysis = f"⚠️ LLM failed: {e}"
    else:
        analysis = f"⚠️ No API key. Delta SDG: {delta.get('composite_sdg_score')}"
